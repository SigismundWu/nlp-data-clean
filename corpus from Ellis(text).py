...
#首先text中所有NA去掉
data_text = data_ff.dropna(subset=["text"])
...
for i in range(len(data_text["text"])):
    if len(data_text["text"][i]) <= 2:
        d_schar_list.append(i)
...
d_chinese_list = []
for i in range(len(data_text["text"])):
    if cc_chinese(data_text.iloc[i]["text"]): 
...
#上面那个问题可以用底层数据结构字典来解决了，大大提升源码性能。
...
final_list_withbreak = final_list
...
for i in range(len(data_text_final)):
    check_list_0.append(data_text_final.iloc[i]["text"] + 
                      "(platform:" + "Ellis" + ")" + 
                      "(type:" + "text" + ")" +
